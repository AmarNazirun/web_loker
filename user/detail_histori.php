...
// jika menekan tombol lamar pekerjaan
if (isset($_POST['lamar_pekerjaan'])) {
    // ambil id_lowongan dan id_calon
    $id_lowongan = $_GET['id_lowongan'];
    $id_calon = $data['id_calon'];
    // cek apakah sudah melamar atau belum
    $check_query = "SELECT * FROM pelamar WHERE id_lowongan = $id_lowongan AND id_calon = $id_calon";
    $check_result = mysqli_query($koneksi, $check_query);
    if (mysqli_num_rows($check_result) > 0) {
        // jika sudah melamar, tampilkan pesan
        echo "<script>alert('Anda sudah melamar pekerjaan ini.');</script>";
    } else {
        // jika belum melamar, insert data ke tabel pelamar
        $insert_query = "INSERT INTO pelamar (tanggal_melamar, id_lowongan, id_calon, status_lamaran) VALUES (NOW(), $id_lowongan, $id_calon, 'Menunggu Konfirmasi')";
        if (mysqli_query($koneksi, $insert_query)) {
            echo "<script>alert('Anda telah berhasil melamar pekerjaan ini.');</script>";
        }
        else {
            echo "<script>alert('Gagal melamar pekerjaan. Silakan coba lagi.');</script>";
        }
    }
}
?>
