        ...
        <section class="section dashboard">
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">Perbaharui Data Pengalaman Organisasi</h5>
                            <?php
                            // query untuk mengambil data pengalaman kerja
                            $query = "SELECT * FROM organisasi_calon WHERE id_organisasi = $_GET[id_organisasi]";
                            $result = mysqli_query($koneksi, $query);
                            $data = mysqli_fetch_assoc($result);
                            ?>
                            ...
                            if (isset($_POST['submit'])) {
                                ...
                                $query = "UPDATE organisasi_calon SET lembaga='$lembaga', bidang='$bidang', tahun_awal='$tahun_awal', tahun_akhir='$tahun_akhir', negara_kota='$negara_kota' WHERE id_organisasi=$_GET[id_organisasi]";
                                if (mysqli_query($koneksi, $query)) {
                                    echo "<script>alert('Data Pengalaman Organisasi berhasil disimpan');</script>";
                                    echo "<script>window.location.href='user_profile.php';</script>";
                                } else {
                                    echo "<script>alert('Gagal menyimpan data Pengalaman Organisasi');</script>";
                                }
                            }
                            ?>
                        </div>
